...
```
Expected = $10,000 (starting) + Claims + Missions (reward_paid) + Referrals (claimed) + total_pnl - Open Collateral
Expected = MAX(0, Expected)  -- Balance cannot go negative
Discrepancy = paper_balance - Expected
```

Note:
- Uses `total_pnl` from users table (current PnL snapshot)
    ...
    SELECT
      ...
      COUNT(*) FILTER (WHERE ABS(
        u.paper_balance - GREATEST(0, 10000 + COALESCE(c.total_claims, 0) + COALESCE(m.total_mission_rewards, 0) +
         ...
         u.total_pnl - COALESCE(oc.collateral, 0))
...
(async () => {
  const result = await pool.query(\`
    ...
    SELECT
      ...
      GREATEST(0, 10000 + COALESCE(c.total_claims, 0) + COALESCE(m.total_mission_rewards, 0) +
        COALESCE(rr.referrer_rewards, 0) + COALESCE(rd.referred_rewards, 0) +
        u.total_pnl - COALESCE(oc.collateral, 0)) as expected_balance,
  ...
  if (result.rows.length === 0) {
    ...
    result.rows.forEach(r => {
      ...
      console.log('  PnL: \$' + Number(r.pnl).toLocaleString() + ' | Claims \$' + Number(r.claims).toLocaleString() + ' | Missions \$' + Number(r.missions).toLocaleString());
    });
  }
...
(async () => {
  ...
  const result = await pool.query(\`
    ...
    FROM user_data u, claims c, missions m, referrals r, open_pos o
  \`, [search]);
  ...
  const expected = Math.max(0, 10000 + Number(u.claims) + Number(u.missions) + Number(u.referrals) + Number(u.total_pnl) - Number(u.open_collateral));
  ...
  console.log('+ Total P&L: \$' + Number(u.total_pnl).toLocaleString());
...
echo ""
echo "This will fix all users to their expected balance."
```
